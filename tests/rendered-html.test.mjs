...
test("Atlas Steward root restores canonical Home without AtlasState or seeded fallback", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  assert.match(page, /\/api\/v1\/health/);
  assert.match(page, /\/api\/v1\/projects/);
  assert.match(page, /\/work/);
  assert.match(page, /No project exists in this workspace yet/);
  assert.doesNotMatch(page, /makeSeedState|\/api\/state|England|Ghana|AtlasState/);
});

test("V1.8 blueprint defers the broader program to Atlas Steward route authority", async () => {
  const blueprint = await readFile(new URL("../docs/V1.8_IMPLEMENTATION_BLUEPRINT.md", import.meta.url), "utf8");
  assert.match(blueprint, /Atlas Steward v0\.1 and its incorporated UI addendum govern the current implementation slice/);
  assert.match(blueprint, /broader V1\.8 evaluation, company-benchmark, retrieval, graph, integration, and platform program.*paused/s);
  assert.match(blueprint, /`\/work` → \*\*Home\*\*/);
...
test("verified-owner authorization is an explicit durable deployment contract", async () => {
  ...
  assert.match(shell, /authorized \? "Signed in" : "Sign in to continue"/);
...
test("Slice 6A Work and conversation actions use canonical services only", async () => {
  ...
  assert.match(shell, /Sign in to continue/);
  assert.match(shell, /signin-with-chatgpt/);
  assert.match(shell, /signout-with-chatgpt/);
  assert.match(shell, /cannot make changes to this Atlas workspace/);
...
test("V1.8 default surface is room-transfer-only while internal ontology remains available under Advanced", async () => {
  const [shell, home, conversation, inspect, contextualAdd, slice6b] = await Promise.all([
    readFile(new URL("../app/components/project-shell.tsx", import.meta.url), "utf8"),
    readFile(new URL("../app/projects/[projectId]/work/work-workspace.tsx", import.meta.url), "utf8"),
    ...
    readFile(new URL("../worker/slice6b-api.ts", import.meta.url), "utf8"),
  ]);
  ...
  for (const internal of ["Cases", "Mechanisms", "Blueprint", "Roadways", "Governance"]) {
    assert.match(inspect, new RegExp(`"${internal}"`));
  }
  assert.match(inspect, /title="Internal records"/);
...
test("Roadway applicability requires positive task, scope, or governed-relationship evidence", async () => {
  const worker = await builtWorker("roadway-positive-applicability");
  const DB = await sqliteD1();
  await seedCanonicalProject(worker, DB, "workflow", "Workflow Engine");
  await slice2Request(worker, DB, "/api/v1/projects/workflow/roadways");

  const unrelated = await slice2Request(worker, DB, "/api/v1/projects/workflow/reconstruction/interpret", {
    method: "POST",
    body: { task: "Continue the accepted project work and preserve its governing constraints." },
  });
  assert.equal(unrelated.response.status, 200);
  assert.equal(unrelated.value.interpretation.primaryRoadway, null);
  assert.equal(unrelated.value.interpretation.materialAmbiguity, false);
  assert.equal(unrelated.value.interpretation.clarificationRequired, false);
  assert.deepEqual(unrelated.value.interpretation.candidateInterpretations, []);
  assert.equal(unrelated.value.interpretation.applicability.applicableRoadwayIds.length, 0);
  assert.equal(unrelated.value.interpretation.applicability.excludedRoadways.length, 3);

  const generic = await slice2Request(worker, DB, "/api/v1/projects/workflow/reconstruction/interpret", {
    method: "POST",
    body: { task: "Find the strongest way to continue this broad project safely." },
  });
  assert.equal(generic.value.interpretation.primaryRoadway, null);
  assert.equal(generic.value.interpretation.materialAmbiguity, false);
  assert.equal(generic.value.interpretation.applicability.applicableRoadwayIds.length, 0);

  const applicable = await slice2Request(worker, DB, "/api/v1/projects/workflow/reconstruction/interpret", {
    method: "POST",
    body: { task: "Evaluate the run line by separating winning from covering at the offered price." },
  });
  assert.equal(applicable.value.interpretation.primaryRoadway.name, "Margin / Run-Line Value");
  assert.equal(applicable.value.interpretation.materialAmbiguity, false);
  assert.equal(applicable.value.interpretation.applicability.applicableRoadwayIds.length, 1);

  const conflicting = await slice2Request(worker, DB, "/api/v1/projects/workflow/reconstruction/interpret", {
    method: "POST",
    body: { task: "Compare the best option and explain why the prior outcome failed." },
  });
  assert.equal(conflicting.value.interpretation.primaryRoadway, null);
  assert.equal(conflicting.value.interpretation.materialAmbiguity, true);
  assert.ok(conflicting.value.interpretation.candidateInterpretations.length >= 2);

  const scopeMismatch = await slice2Request(worker, DB, "/api/v1/projects/workflow/reconstruction/interpret", {
    method: "POST",
    body: { task: "Perform only an outcome postmortem; do not price the current margin thesis." },
  });
  assert.equal(scopeMismatch.value.interpretation.primaryRoadway.name, "Outcome / Postmortem");
  assert.ok(scopeMismatch.value.interpretation.applicability.excludedRoadways.some((roadway) => (
    roadway.name === "Margin / Run-Line Value"
      && /non-applicable task pattern/i.test(roadway.reason)
  )));

  const conversation = await slice2Request(worker, DB, "/api/v1/projects/workflow/conversations", {
    method: "POST",
    body: { title: "Relationship applicability" },
  });
  const caseResult = await slice2Request(worker, DB, "/api/v1/projects/workflow/cases", {
    method: "POST",
    body: {
      objective: "Apply the governed edge rule to the current recommendation",
  ...
  seedSlice4Mechanism(DB, {
    id: "mechanism:relationship-roadway-proof",
    projectId: "workflow",
    statement: "The governed edge rule evaluates run-line margin and covering rather than outright winning alone.",
    supportingCaseIds: [caseResult.value.case.id],
  });
  const relationshipBacked = await slice2Request(worker, DB, "/api/v1/projects/workflow/reconstruction/interpret", {
    method: "POST",
    body: {
      task: "Apply the governed edge rule to the current recommendation.",
      caseId: caseResult.value.case.id,
    },
  });
  assert.equal(relationshipBacked.value.interpretation.primaryRoadway.name, "Margin / Run-Line Value");
  assert.equal(relationshipBacked.value.interpretation.applicability.governedRelationshipEvidenceUsed, true);
});
...
test("Slice 4 comparable packets record the exact Slice 3 governance event that changes eligibility", async () => {
  ...
  assert.ok(
    after.value.receipt.treatmentSummary.Use.some(
      (item) => item.sourceId === seeded.events[0].id && /counterevidence|uncertainty|provenance/i.test(item.reason),
    ),
  );
  assert.ok(
...
test("Slice 4 calibration treatments enforce mechanism, scope, authority, evidence, conflict, and project gates", async () => {
  ...
  assert.equal(byId.get(correction.id).treatment, expectations.user_correction_vs_repeated_inference.expectedTreatment);
  assert.match(byId.get(correction.id).reason, /uncertainty|provenance/i);
  assert.match(narrow.value.packet.compiledContent, /Strong counterexample/i);
  assert.equal(narrow.value.packet.compiledContent.includes("Milwaukee Brewers uniform colors are blue and gold"), false);
  assert.doesNotMatch(narrow.value.packet.compiledContent, /\[CONSIDER\]|\[EXCLUDE\]/);
...
test("Slice 4 preserves Brewers Reconstructed limitations beside an Exact native source", async () => {
  ...
  assert.equal(packet.value.packet.compiledContent.includes(reconstructed.statement), false);
  assert.doesNotMatch(packet.value.packet.compiledContent, /historical raw transcript unavailable; not Exact/i);
  assert.equal(
...
test("Slice 5 causal receipt connects a comparable packet diff to the exact governance event", async () => {
  ...
  assert.equal(independentCorrection.treatment, "Use");
  assert.equal(independentCorrection.packetEligibleProtected, true);
  assert.match(independentCorrection.reason, /uncertainty|provenance/i);
...
test("Slice 5 preserves Brewers as Reconstructed beside Exact native source through handoff", async () => {
  ...
  assert.equal(exact.representation, "Exact");
  assert.equal(handoff.value.packet.compiledContent.includes(reconstructed.statement), false);
  assert.doesNotMatch(handoff.value.packet.compiledContent, /historical raw transcript unavailable; not Exact/i);
...
test("Dogfood preview uses canonical case context and collapses governed lineage without mutation", async () => {
  ...
  assert.equal(preview.value.treatmentSummary.Use.length, 2);
  const mechanism = preview.value.treatmentSummary.Use.find((item) => item.sourceId === mechanismId);
  assert.ok(mechanism);
  ...
  const challenge = preview.value.treatmentSummary.Use.find((item) => item.sourceId === seeded.events[2].id);
  assert.ok(challenge);
  assert.match(challenge.reason, /counterevidence|uncertainty|provenance/i);
...
test("Atlas Steward is focused, project-resetting, mobile-capable, and free of production fixtures", async () => {
  const [
    workspace,
    aid,
    candidates,
...
test("Slice 6B interface remains canonical and explicit beneath the Slice 6C Ask completion", async () => {
  ...
  for (const text of [
    "Atlas needs your decision",
    "What should carry forward",
    "Your wording",
    "Do not carry this forward",
    "Atlas will ask again when",
  ...
  assert.doesNotMatch(shell, /ContextualAdd|Open Contextual Add/);
  assert.match(contextualAdd, /Contextual Add/);
...
test("Atlas Steward persists Light and Full through the same reconstruction engine", async () => {
  ...
  await seedCanonicalProject(worker, DB, "workflow", "Workflow Engine");
  await initializeRoadways(worker, DB, "workflow");
  const statement = "When Cody requests a Codex-ready transfer from mobile, use concise plain text that can be copied directly.";
  seedSlice4Mechanism(DB, {
    id: "mechanism:steward-light-to-full",
    projectId: "workflow",
    statement,
  });
  ...
  const full = await reconstructionRunRequest(worker, DB, "workflow", {
    ...
    roadwayOverride: broadRoadway.id,
  }, "steward-full-escalation-clarified");
  ...
  assert.equal(full.value.literalTask, "Prepare a Codex-ready transfer from mobile.");
  assert.equal(full.value.capsule, null);
  assert.ok(full.value.packet.id);
