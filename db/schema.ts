...
export const handoffs = sqliteTable("handoffs", {
  ...
  packetSnapshotHash: text("packet_snapshot_hash"),
  primaryRoadwayId: text("primary_roadway_id"),
  primaryRoadwayVersionId: text("primary_roadway_version_id"),
...
export const handoffLifecycleEvents = sqliteTable("handoff_lifecycle_events", {
  id: text("id").primaryKey(),
  projectId: text("project_id").notNull().references(() => projects.id),
  handoffId: text("handoff_id").notNull().references(() => handoffs.id),
  status: text("status").notNull(),
  ...
  metadata: text("metadata").notNull().default("{}"),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("handoff_lifecycle_status").on(table.handoffId, table.status)]);

export const handoffAnswers = sqliteTable("handoff_answers", {
  id: text("id").primaryKey(),
  projectId: text("project_id").notNull().references(() => projects.id),
  handoffId: text("handoff_id").notNull().references(() => handoffs.id),
  packetId: text("packet_id").notNull().references(() => packets.id),
  providerResponseId: text("provider_response_id").notNull(),
  receivingProvider: text("receiving_provider").notNull(),
  receivingModel: text("receiving_model").notNull(),
  answerText: text("answer_text").notNull(),
  answerTimestamp: text("answer_timestamp").notNull(),
  canonicalMessageReference: text("canonical_message_reference"),
  metadata: text("metadata").notNull().default("{}"),
}, (table) => [uniqueIndex("handoff_answers_handoff").on(table.handoffId)]);
...
export const handoffReceipts = sqliteTable("handoff_receipts", {
  ...
  handoffId: text("handoff_id").notNull().references(() => handoffs.id),
  packetId: text("packet_id").notNull().references(() => packets.id),
  packetReceiptId: text("packet_receipt_id").notNull().references(() => receipts.id),
  lineage: text("lineage").notNull().default("[]"),
  treatmentSummary: text("treatment_summary").notNull().default("{}"),
  authorityAndScope: text("authority_and_scope").notNull().default("{}"),
  freshnessSummary: text("freshness_summary").notNull().default("{}"),
  inferenceDisclosure: text("inference_disclosure"),
  unresolvedConflicts: text("unresolved_conflicts").notNull().default("[]"),
  governanceCauses: text("governance_causes").notNull().default("[]"),
  packetDifference: text("packet_difference").notNull().default("[]"),
  additionalLiveRetrieval: text("additional_live_retrieval").notNull().default("{}"),
  finalAnswerReference: text("final_answer_reference"),
  historicalLimitations: text("historical_limitations").notNull().default("[]"),
  honestyStatement: text("honesty_statement").notNull(),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [uniqueIndex("handoff_receipts_handoff").on(table.handoffId)]);
