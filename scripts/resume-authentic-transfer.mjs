...
workerUrl.searchParams.set("authentic-proposition-resume", `${process.pid}-${Date.now()}`);
...
const env = {
  ...
  CAMPUS_ATLAS_DEPLOYMENT_VERSION: "local-authentic-proposition-repair",
  CAMPUS_ATLAS_SOURCE_COMMIT: "working-tree-authentic-proposition-repair",
};
...
const transfer = await request(
  ...
  { method: "POST", idempotencyKey: "authentic-full-transfer-run-001-proposition-repair-resume" },
);
...
const evidence = {
  ...
  phase: "pre_governance_after_mature_room_proposition_repair",
...
const evidencePath = path.join(outputDir, "transfer-result-after-proposition-repair.json");
