...
function CaptureHub({ defaultProject, nodes, onSubmit, onBuildPacket, onClose }: { defaultProject: ProjectKey; nodes: KnowledgeNode[]; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onBuildPacket: (task: string, project: ProjectKey) => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="capture-hub" onMouseDown={(event) => event.stopPropagation()}><div className="modal-header"><div><p>{projectFor(projectKey).label} · project capture</p><h2>What are you adding?</h2><small>Capture stays inside this project. Atlas connects it only when the evidence earns that connection.</small></div><button type="button" onClick={onClose}>×</button></div><div className="capture-action-grid">{captureActions.map((item) => <button type="button" className={action === item.key ? "active" : ""} onClick={() => { setAction(item.key); setFormError(""); }} key={item.key}><span>{item.icon}</span><div><strong>{item.title}</strong><p>{item.copy}</p></div></button>)}</div>{action === "packet" ? <section className="capture-form packet-entry"><div><span className="form-kicker">Build Context Packet</span><h3>Assemble the smallest useful context for a new task.</h3></div><label>New task or question<textarea rows={4} value={packetTask} onChange={(event) => setPacketTask(event.target.value)} /></label><button className="primary-button wide" disabled={packetTask.trim().length < 8} onClick={() => onBuildPacket(packetTask, projectKey)}>Build Context Packet →</button></section> : <form className="capture-form" noValidate onSubmit={(event) => { const form = event.currentTarget; if (!form.checkValidity()) { event.preventDefault(); const invalid = form.querySelector(":invalid") as HTMLElement | null; setFormError("Complete the highlighted field before saving."); invalid?.scrollIntoView({ block: "center" }); invalid?.focus(); return; } setFormError(""); onSubmit(event); }}><input type="hidden" name="action" value={action} /><input type="hidden" name="project" value={projectKey} /><div className="capture-form-heading"><span className="form-kicker">{actionTitle}</span><h3>{action === "thesis" ? "Create a new knowledge object" : "Update an existing knowledge object"}</h3><p>{action === "thesis" ? "The entry begins without promotion or retrieval authority." : "Choose the existing thesis this evidence belongs to. The update will not create a duplicate."}</p></div>{needsTarget && <label>Attach this to<select name="targetId">{targetNodes.map((node) => <option value={node.id} key={node.id}>{node.title}</option>)}</select></label>}<label>{action === "thesis" ? "Research or thesis title" : action === "outcome" ? "What happened in reality?" : action === "postmortem" ? "Post-mortem conclusion" : action === "correction" ? "What needs correction?" : "Evidence summary"}<input name="title" required /></label><label>{action === "thesis" ? "Reasoning, evidence, assumptions, and missing information" : "What happened, why it matters, and what should change"}<textarea name="details" required rows={5} /></label><div className="form-row"><label>Source<input name="source" required placeholder="Conversation, result, screenshot, research…" /></label><label>Source fidelity<select name="fidelity"><option>Exact</option><option>Reconstructed</option><option>Inferred</option></select></label></div>{action !== "thesis" && <label>Effect on the existing knowledge<select name="impact"><option value="challenge">Challenge</option><option value="support">Reinforce</option>{action === "correction" && <option value="supersede">Supersede</option>}</select></label>}<label>{action === "thesis" ? "Optional connection" : "Optional additional connection"}<select name="relatedId"><option value="">No additional connection</option>{nodes.filter((node) => !needsTarget || !targetNodes.some((target) => target.id === node.id)).map((node) => <option value={node.id} key={node.id}>{node.title}</option>)}</select></label><label>Why this connection matters<textarea name="connectionReason" rows={2} defaultValue="The shared mechanism may affect how this case is researched later." /></label>{formError && <div className="capture-error" role="alert">{formError}</div>}<div className="capture-rule"><span>i</span><p>Saving always creates a visible receipt. Optional connections begin as inferred and require approval.</p></div><button className="primary-button wide" disabled={needsTarget && targetNodes.length === 0}>Save and inspect Connection Receipt →</button></form>}</section></div>;
}
