...
type SportsWorkspaceView = "cases" | "knowledge" | "workbench" | "blueprint";
...
function SportsEngineV43({ nodes, reviews, connections, handoffResult, handoffStatus, lastCapture, onBack, onCapture, onBuildPacket, onInspectKnowledge, onPromote, toast }: {
  nodes: KnowledgeNode[];
  reviews: ReviewEvent[];
  ...
  const workbenchActions = captureActions.filter((action) => action.key !== "packet");
  ...
  return <main className="sports-workspace-shell">
    <header className="sports-workspace-topbar">
      ...
      <div className="sports-workspace-actions"><span className="workspace-live"><i /> Project state live</span><button className="ghost-button" onClick={onBack}>← Amy Campus</button></div>
    </header>
    ...
    <section className="sports-workspace-hero">
      ...
      <button className="primary-button sports-new-case" onClick={() => onCapture("thesis")}><span>＋</span> New case</button>
    </section>
    ...
    <nav className="sports-workspace-tabs" aria-label="Sports Engine workspace areas">
      {([
        ...
        ["workbench", "Workbench", "Capture · test · retrieve"],
    ...
    {view === "cases" && selectedCase && <section className="sports-view sports-cases-view">
      <aside className="case-index">
        ...
        <div className="case-list">{cases.map((item) => {
          const complete = caseIsComplete(item);
          const itemReviews = reviews.filter((review) => review.nodeId === item.id);
          return <button key={item.id} className={selectedCase.id === item.id ? "active" : ""} onClick={() => setSelectedCaseId(item.id)}><div><span className={complete ? "case-status complete" : "case-status active"}>{complete ? "Completed" : "Active"}</span><small>{item.room} · {fidelityFor(item)}</small></div><strong>{item.title}</strong><p>{item.summary}</p><footer><span>{itemReviews.length} evidence events</span><span>{connections.filter((edge) => edge.from === item.id || edge.to === item.id).length} connections</span></footer></button>;
        })}</div>
      ...
      <article className="case-record">
        <header className="case-record-header"><div><div className="case-record-meta"><span className={caseIsComplete(selectedCase) ? "case-status complete" : "case-status active"}>{caseIsComplete(selectedCase) ? "Completed case" : "Active case"}</span><small>{selectedCase.room} · {fidelityFor(selectedCase)} source</small></div><h2>{selectedCase.title}</h2><p>{selectedCase.summary}</p></div><div className="case-quick-actions"><button onClick={() => onCapture("outcome", selectedCase.id)}>Record outcome</button><button onClick={() => onCapture("postmortem", selectedCase.id)}>Run post-mortem</button><button onClick={() => onCapture("evidence", selectedCase.id)}>＋ Evidence</button></div></header>
        <div className="case-spine">
          ...
          <CaseRecordSection number="08" title="Future retrieval influence"><div className="retrieval-receipt"><header><div><span>Retrieved because</span><strong>{connections.some((edge) => edge.approved && (edge.from === selectedCase.id || edge.to === selectedCase.id)) ? "Eligible through approved paths" : "Waiting for approved connection"}</strong></div><b>{(confidenceFor(selectedCase.id, reviews) / 100).toFixed(2)}</b></header><div><span>✓ Same project</span><span>✓ Shared mechanism</span><span>✓ Frequently co-activated</span><span>✓ High reconstruction value</span></div><p>Later packets can use this case only with its outcome, challenge history, connection reason, and source fidelity attached.</p></div></CaseRecordSection>
        </div>
      </article>
    </section>}
    ...
    {view === "workbench" && <section className="sports-view sports-workbench-view">
      <div className="workbench-main"><div className="workspace-section-heading"><div><p>Workbench</p><h2>Do the work inside the project</h2><span>Every action updates Cases, Knowledge, or Blueprint state. Nothing disappears into a receipt-only flow.</span></div></div><div className="workbench-action-grid">{workbenchActions.map((action) => <button key={action.key} onClick={() => onCapture(action.key, selectedCase?.id)}><span>{action.icon}</span><div><strong>{action.title}</strong><p>{action.copy}</p></div><i>Open →</i></button>)}</div><div className="workbench-recent"><div className="workspace-section-heading compact"><div><p>Recent Work</p><h3>Visible project changes</h3></div><button onClick={() => setView("cases")}>Open cases →</button></div>{recentWork.map((item) => <article key={`${item.id}-workbench`}><span>{item.label}</span><div><strong>{item.node.title}</strong><p>{item.detail}</p></div><small>{item.date}</small></article>)}</div></div>
      <aside className="packet-workbench"><span className="form-kicker">Context Packet</span><h2>Test what the project gives back.</h2><p>Ask a later question and inspect whether completed cases and promoted knowledge actually change the handoff.</p><label>Later question<textarea rows={5} value={packetTask} onChange={(event) => setPacketTask(event.target.value)} /></label><button className="primary-button wide" disabled={handoffStatus === "building" || packetTask.trim().length < 8} onClick={() => onBuildPacket(packetTask)}>{handoffStatus === "building" ? "Building from project state…" : "Build from Sports Engine →"}</button>{handoffStatus === "error" && <div className="inline-error">The packet could not be built. Your question is still here.</div>}{handoffResult && <div className="workbench-packet-result"><header><span>Latest packet</span><strong>{handoffResult.packetId}</strong></header><div>{handoffResult.durableKnowledge.slice(0, 3).map((item) => <article key={item.id}><b>{item.usefulness}</b><div><strong>{item.title}</strong><p>{item.whyIncluded}</p><small>{item.fidelity} · {item.confidence}% confidence</small></div></article>)}</div><footer><span>{handoffResult.budget.used}/{handoffResult.budget.limit} items</span><span>~{handoffResult.budget.estimatedTokens} tokens</span></footer></div>}</aside>
    </section>}

    {view === "blueprint" && <section className="sports-view sports-blueprint-view">
      <div className="blueprint-operating-rule"><div><p>Project Blueprint · live methodology</p><h2>What completed cases have taught Sports Engine to do.</h2></div><span>{promotedKnowledge.length} principles currently hold retrieval authority</span></div><div className="blueprint-method">{["Research", "Audit", "Price", "Decide", "Outcome", "Post-mortem", "Promote"].map((step, index) => <div key={step}><span>{String(index + 1).padStart(2, "0")}</span><strong>{step}</strong>{index < 6 && <i>→</i>}</div>)}</div><div className="blueprint-content"><section><div className="workspace-section-heading"><div><p>Capability ledger</p><h2>Evidence behind each earned capability</h2></div><b>{sportsCapabilities.length}</b></div><div className="capability-ledger v43">{sportsCapabilities.map((capability, index) => <details key={capability.name} open={index === 3 || index === 4}><summary><span>{String(index + 1).padStart(2, "0")}</span><strong>{capability.name}</strong><i>Inspect evidence</i></summary><div><p><b>Blueprint rule</b>{capability.rule}</p><p><b>Evidence earned from</b>{capability.evidence}</p><p><b>Last tested</b>{capability.tested}</p><p><b>Current limitation</b>{capability.limit}</p><p><b>Next proof required</b>{capability.next}</p></div></details>)}</div></section><aside><div className="workspace-section-heading"><div><p>Blueprint changes</p><h2>Promotion pipeline</h2></div></div>{candidateKnowledge.map((item) => { const metrics = metricsFor(item.id, reviews); return <article className="blueprint-candidate" key={item.id}><span>{item.level}</span><strong>{item.title}</strong><p>{metrics.unresolved ? `${metrics.unresolved} unresolved challenge blocks promotion.` : "Evidence is ready for human review."}</p><div><small>{metrics.supporting} support</small><small>{metrics.challenging} challenge</small><small>{item.sourceFidelity}% fidelity</small></div><button onClick={() => onPromote(item.id)}>Review evidence and lineage →</button></article>; })}<div className="blueprint-rule"><span>Constitutional rule</span><p>No capability is earned because a number increased. Every Blueprint change must reconstruct the cases, evidence, challenges, and approval that produced it.</p></div></aside></div>
    </section>}
